...
public class TencentWeiboErrorHandler extends DefaultResponseErrorHandler {

    private  static final String TENCENT_WEIBO_PROVIDER_ID = "tencent";
    ...
    public void handleError(ClientHttpResponse response) throws IOException {
        ...
        try {
            ...
            throw new UncategorizedApiException(TENCENT_WEIBO_PROVIDER_ID, "Error consuming Tencent REST API", e);
        }
    }
    ...
    private void handleClientErrors(ClientHttpResponse response)
        ...
        if (errorMap.containsKey("error_code")) {
            ...
            switch (errorCode) {
                case 21327:
                    throw new ExpiredAuthorizationException(TENCENT_WEIBO_PROVIDER_ID);
                default:
                    throw new UncategorizedApiException(TENCENT_WEIBO_PROVIDER_ID, String.format(
                            "Error calling Tencent: error code=%d, error message=%s",
    ...
    private void handleServerErrors(HttpStatus statusCode) throws IOException {
        if (statusCode == HttpStatus.INTERNAL_SERVER_ERROR) {
            throw new InternalServerErrorException(TENCENT_WEIBO_PROVIDER_ID, "Something is broken at Tencent.");
        } else if (statusCode == HttpStatus.BAD_GATEWAY) {
            throw new ServerDownException(TENCENT_WEIBO_PROVIDER_ID, "Tencent is down or is being upgraded.");
        } else if (statusCode == HttpStatus.SERVICE_UNAVAILABLE) {
            throw new ServerOverloadedException(TENCENT_WEIBO_PROVIDER_ID, "Tencent is overloaded with requests. Try again later.");
        }
    }
